...
class  TDatesTest extends TestCase
{
    ...
    // todo: fix tests. code is working reliably in two production apps
    /*
    ...
    public function testLocalDateTimeToUTC()
    {
        $nowStr = (new \DateTime())->format('Y-m-d H:i:s');
        $expected = TDates::LocalDateTimeToUTC($nowStr, 'america/chicago');
        $actual = TDates::localDateTimeToUTC($nowStr, -360);
        $expectedFormated = $expected->format('Y-m-d H:i');
        $actualFormated = $actual->format('Y-m-d H:i');
        $this->assertEquals(
            $expectedFormated,
            $actualFormated);
        $nowStr = (new \DateTime())->format('Y-m-d H:i:s');
        $actual = TDates::LocalDateTimeToUTC($nowStr, '-06:00');
        $expected = TDates::LocalDateTimeToUTC($nowStr, 'america/chicago');
        $expectedFormated = $expected->format('Y-m-d H:i');
        $actualFormated = $actual->format('Y-m-d H:i');
        $this->assertEquals(
